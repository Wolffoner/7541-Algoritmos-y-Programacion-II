 ...
 */
void nodo_set_elemento(nodo_abb_t** nodo, void* elemento){
    if(!nodo)
        return;
 ...
 */
void nodo_destruir(abb_liberar_elemento destructor, nodo_abb_t* nodo_actual ){
    if(!nodo_actual) 
        return;
    nodo_destruir(destructor, nodo_actual->izquierda);
    nodo_destruir(destructor, nodo_actual->derecha);
    if(destructor)
 ...
 */
int nodo_borrar(abb_t* arbol, nodo_abb_t* nodo, nodo_abb_t** nodo_previo, void* elemento){
    ...
    if(arbol->comparador(elemento, nodo->elemento) == COMPARADOR_BUSCADO){
        ...
        nodo_destruir(arbol->destructor, nodo_encontrado);
        return VALIDO;
    } else if(arbol->comparador(elemento, nodo->elemento) < COMPARADOR_BUSCADO){
        *nodo_previo = nodo;
        return nodo_borrar(arbol, nodo->izquierda, nodo_previo, elemento);
    } else if(arbol->comparador(elemento, nodo->elemento) > COMPARADOR_BUSCADO){
        *nodo_previo = nodo;
        return nodo_borrar(arbol, nodo->derecha, nodo_previo, elemento);
    }
    return ERROR;
}
...
int arbol_insertar(abb_t* arbol, void* elemento){
    ...
    if(!nodo)
        return ERROR;
    nodo_set_elemento(&nodo, elemento);
...
int arbol_borrar(abb_t* arbol, void* elemento){
    ...
    nodo_abb_t* nodo_previo = NULL;
    nodo_borrar(arbol, arbol->nodo_raiz, &nodo_previo, elemento);
    return VALIDO;
} 
...
void arbol_destruir(abb_t* arbol){
    if(arbol){
        nodo_destruir(arbol->destructor, arbol->nodo_raiz);
        free(arbol);
    }
}
